...
def main() -> None:
    ...
    df_prices = df_prices.rename(columns={"Date": "date", "Close": "close", "TRDPRC_1": "close"})
    ...
    print(f"Ljung-Box (Sq Return, Lag 5): Q={lb_sq_ret[5][0]:.4f}, p={lb_sq_ret[5][1]:.6e}")
    print(f"Ljung-Box (Sq Return, Lag 10): Q={lb_sq_ret[10][0]:.4f}, p={lb_sq_ret[10][1]:.6e}")
    ...
    print(f"Positive Articles: {pos_articles} ({pos_articles/n_sent_articles*100:.2f}%)")
    print(f"Negative Articles: {neg_articles} ({neg_articles/n_sent_articles*100:.2f}%)")
    print(f"Neutral Articles: {neu_articles} ({neu_articles/n_sent_articles*100:.2f}%)")
    ...
    print(f"Zero-news trading days: {zero_news_days} ({(zero_news_days / len(df_daily)) * 100:.4f}%)")
