...
from src.modeling.dataset import build_model_frame, compute_volatility_features
...
def plot_return_distribution(prices_path: str | Path, output_dir: Path) -> None:
    ...
    count, bins, ignored = ax.hist(
        returns, bins=60, density=True, alpha=0.6, color="#1f77b4", edgecolor="#15537e", label="Returns"
    )
...
def plot_volatility_clustering(prices_path: str | Path, output_dir: Path) -> None:
    ...
    ax2.axvspan(pd.Timestamp("2020-03-01"), pd.Timestamp("2020-05-31"), color="#d62728", alpha=0.1)
    ax2.text(pd.Timestamp("2020-04-15"), vol.max() * 0.9, "COVID-19", color="#d62728", weight="bold", ha="center")
    
    # Liquidity crash (April 2022)
    ax2.axvspan(pd.Timestamp("2022-04-01"), pd.Timestamp("2022-06-30"), color="#d62728", alpha=0.1)
    ax2.text(pd.Timestamp("2022-05-15"), vol.max() * 0.9, "Liquidity Shock", color="#d62728", weight="bold", ha="center")
...
def plot_forecast_comparison(summary_path: str | Path, output_dir: Path) -> None:
    ...
    ax.plot(dates, actual, color="#7f7f7f", alpha=0.7, linewidth=1.0, label="Realized Parkinson Volatility")
    ax.plot(dates, baseline, color="#d62728", alpha=0.9, linewidth=1.2, linestyle="--", label="GARCH(1,1) Baseline")
    ax.plot(dates, hybrid, color="#1f77b4", alpha=0.9, linewidth=1.5, label="Hybrid GARCH + Sentiment LSTM")
