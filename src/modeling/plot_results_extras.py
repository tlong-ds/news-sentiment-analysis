...
def plot_sentiment_distribution(sentiment_path: str | Path, output_dir: Path) -> None:
    ...
    counts, bins, patches = ax.hist(
        scores, bins=50, alpha=0.7, color="#2ca02c", edgecolor="#1e6f1e", label="Sentiment Scores"
    )
    
    # Add vertical lines for thresholds
    ax.axvline(0.05, color="#d62728", linestyle="--", linewidth=1.5, label="Positive Threshold (0.05)")
    ax.axvline(-0.05, color="#1f77b4", linestyle="--", linewidth=1.5, label="Negative Threshold (-0.05)")
...
def plot_robustness_comparison(summary_path: str | Path, output_dir: Path) -> None:
    ...
    for label, key in specs.items():
        if key in summary:
            ...
            garch_rmse.append(summary[key]["baseline_rmse"] * 100)  # scale to percentage points
    ...
    rects1 = ax.bar(x - width/2, garch_rmse, width, label="GARCH Baseline", color="#d62728", alpha=0.85)
    rects2 = ax.bar(x + width/2, hybrid_rmse, width, label="Hybrid Model", color="#1f77b4", alpha=0.85)
    
    ax.set_ylabel("Out-of-Sample RMSE (%)")
    ax.set_title("Volatility Forecast Performance (RMSE) Across Robustness Specifications")
    ax.set_xticks(x)
