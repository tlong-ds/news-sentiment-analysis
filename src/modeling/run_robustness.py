...
def run_garchx(args: argparse.Namespace) -> dict[str, float]:
    ...
    model_df = model_df.dropna(subset=["log_return", "target_next_vol"]).reset_index(drop=True)

    # Split index
    test_start = pd.Timestamp(args.val_end)
    train_df = model_df[pd.to_datetime(model_df["date"]) <= test_start]
    test_df = model_df[pd.to_datetime(model_df["date"]) > test_start]

    # Fit GARCH-X on training set
    garchx_fit = fit_garchx11_baseline(
        train_df["log_return"],
        exog=train_df["lag_mean_sentiment"]
    )
    ...
    forecast_vol[0] = np.sqrt(max(sample_var, omega / max(1e-6, 1.0 - alpha - beta))) / 100.0
    for idx in range(1, len(y)):
        forecast_vol[idx] = np.sqrt(omega + alpha * y[idx - 1] ** 2 + beta * sigma2[idx - 1] + gamma * x_exog[idx - 1]) / 100.0
    ...
    for idx in range(1, len(y_baseline)):
        sigma2_b[idx] = omega_b + alpha_b * y_baseline[idx - 1] ** 2 + beta_b * sigma2_b[idx - 1]
    
    forecast_b = np.empty_like(sigma2_b)
    forecast_b[0] = np.sqrt(max(sample_var, omega_b / max(1e-6, 1.0 - alpha_b - beta_b))) / 100.0
    for idx in range(1, len(y_baseline)):
        forecast_b[idx] = np.sqrt(omega_b + alpha_b * y_baseline[idx - 1] ** 2 + beta_b * sigma2_b[idx - 1]) / 100.0
...
def run_expanding_garch_eval(args: argparse.Namespace) -> dict[str, float]:
    ...
    model_df["hybrid_residual_target"] = model_df["target_next_vol"] - model_df["garch_forecast_vol"]
...
def main() -> None:
    ...
    results["baseline"] = run_spec(args, sentiment_threshold=0.05, target_type="parkinson", use_sentiment_features=True)

    # 2. Alternative threshold: 0.10
    logger.info("Running Alternative Threshold (0.10)...")
    results["threshold_0.10"] = run_spec(args, sentiment_threshold=0.10, target_type="parkinson", use_sentiment_features=True)

    # 3. Alternative threshold: 0.15
    logger.info("Running Alternative Threshold (0.15)...")
    results["threshold_0.15"] = run_spec(args, sentiment_threshold=0.15, target_type="parkinson", use_sentiment_features=True)

    # 4. Alternative target: Garman-Klass
    logger.info("Running Garman-Klass Volatility target...")
    results["garman_klass"] = run_spec(args, sentiment_threshold=0.05, target_type="garman_klass", use_sentiment_features=True)

    # 5. Ablation study: no sentiment
    logger.info("Running Ablation Study (No Sentiment)...")
    results["no_sentiment_ablation"] = run_spec(args, sentiment_threshold=0.05, target_type="parkinson", use_sentiment_features=False)

    # 6. GARCH-X Baseline
    logger.info("Running GARCH-X Exogenous Sentiment Baseline...")
    results["garch_x"] = run_garchx(args)

    # 7. Expanding Window GARCH
    logger.info("Running Expanding Window GARCH Re-estimation...")
    results["expanding_garch"] = run_expanding_garch_eval(args)

    # Output Results JSON
    output_path = Path(args.output)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps(results, indent=2), encoding="utf-8")
    logger.info(f"Robustness results written to {args.output}")

    # Generate Markdown Table
    print("\n" + "=" * 80)
    print("PHASE 7 ROBUSTNESS CHECKS COMPARISON TABLE")
    print("=" * 80)
    print("| Specification | GARCH RMSE | Hybrid RMSE | GARCH MAE | Hybrid MAE | DM Stat | DM p-value |")
    print("| :--- | :---: | :---: | :---: | :---: | :---: | :---: |")
    for name, r in results.items():
        print(
            f"| **{name}** | {r['baseline_rmse']:.6f} | {r['hybrid_rmse']:.6f} | "
            f"{r['baseline_mae']:.6f} | {r['hybrid_mae']:.6f} | "
            f"{r['dm_stat']:.4f} | {r['dm_pvalue']:.5f} |"
        )
    print("=" * 80)
