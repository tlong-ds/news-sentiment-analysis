...
def test_vnstock_fetch_ticker_news_uses_fallback_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.ingestion.vnstock_source._fetch_from_provider", fake_fetch_from_provider
    )
...
def test_vnstock_fetch_from_provider_marks_pagination_cap(monkeypatch):
    class FakeCompany:
        ...
        def news(self, *, page, page_size):
            ...
            return pd.DataFrame(
                [
                    {
                        "head": f"headline-{self.calls}-{idx}",
                        "publish_time": "2024-12-31T08:30:00",
                    }
    ...
    monkeypatch.setattr(
        "src.ingestion.vnstock_source._make_company",
        lambda symbol, provider: FakeCompany(),
    )

    df, error, capped = __import__(
        "src.ingestion.vnstock_source", fromlist=["_fetch_from_provider"]
...
def test_run_vnstock_counts_provider_failures_and_fallbacks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "src.ingestion.vnstock_source.SOURCE_OUTPUTS",
        {"vnstock": "news_VN_vnstock.csv"},
    )
    monkeypatch.setattr(
        "src.ingestion.vnstock_source.append_articles", lambda *args, **kwargs: None
    )
    ...
    monkeypatch.setitem(
        ...
        "time",
        type("FakeTime", (), {"sleep": staticmethod(fake_sleep)}),
    )
    monkeypatch.setattr(
        "src.ingestion.vnstock_source._fetch_ticker_news", fake_fetch_ticker_news
    )
    monkeypatch.setattr(
